...
export default async function EditExpensePage({
  ...
  if (DEV_MODE) {
    return <EditExpenseForm expense={MOCK_EXPENSE} allGroups={MOCK_GROUPS} userId="u1" />;
  }
  ...
  if (!expense) notFound();
  if (expense.created_by !== user.id) redirect(`/activity/${expenseId}`);

  const allGroups = await getMyGroups(user.id);
  if (!allGroups) redirect(`/activity/${expenseId}`);

  return <EditExpenseForm expense={expense} allGroups={allGroups} userId={user.id} />;
}
